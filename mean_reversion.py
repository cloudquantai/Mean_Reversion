...
class breakout_purchase(Strategy):
    ...
    def register_event_streams(cls, md, service, account):
        return {'!sentiment/bloomberg/story/news': 'on_bloomberg_news', '!sentiment/stocktwits': 'on_stocktwits',
                '!sentiment/alexandria': 'on_alexandria_news'}

    @classmethod
    def is_symbol_qualified(cls, symbol, md, service, account):

        handle_list = service.symbol_list.get_handle(
            '9a802d98-a2d7-4326-af64-cea18f8b5d61')  # this is all stocks on S&P500
            ...
            if len(bar_close) > 0 and bar_close[0] != 0:
                ...
                if self.IsPositionOn == True:
                    ...
                    if (average_low - sell_range_ratio * average_range) > bar_close[0]:
                        ...
                        print(
                            'stock dropped below acceptable values on long, alexandria sentiment ' + str(self.al_value) \
                            + ', bloomberg sentiment ' + str(self.bb_value) + ', stock twit sentiment ' + str(
                                self.st_value))
                    ...
                    # we've made our target profit, let's back out of the trade now
                        ...
                        # send order; use a variable to accept the order_id that order.algo_buy returns
                        sell_order_id = order.algo_sell(self.symbol, algorithm=lime_midpoint_limit_sell, intent="exit")
                        print('selling out of {0} at {1} due to stock reaching target profit at {2}'.format(self.symbol,
                ...
                if self.IsShort == True:
                    ...
                    if average_high + sell_range_ratio * average_range < bar_close[0]:
                        ...
                        self.IsShort = False
                        # send order; use a variable to accept the order_id that order.algo_buy returns
                        order_id = order.algo_buy(self.symbol, algorithm=lime_midpoint_limit_buy, intent="exit")
                        print('exiting short of {0} at {1} due to rising above average low at {2}'.format(self.symbol,
                                                                                                          service.time_to_string(
                                                                                                              service.system_time),
                                                                                                          bar_close[0]))
                        self.IsPurchasable = False
                    ...
                    if average_high - sell_range_ratio * average_range > bar_close[0]:
                        ...
                        order_id = order.algo_buy(self.symbol, algorithm=lime_midpoint_limit_buy, intent="exit")
                        print(
                            'exiting short by buying shares of {0} at {1} due to stock dropping below sell point at {2}'.format(
                                self.symbol, service.time_to_string(service.system_time), bar_close[0]))
                        self.IsPurchasable = False
                    ...
                    if self.IsPositionOn == False and self.IsShort == False and self.IsPurchasable == True:
                        ...
                        if average_low > bar_close[0]:
                            ...
                            order_id = order.algo_buy(self.symbol, algorithm=lime_midpoint_limit_buy,
                                                      price=md[self.symbol].L1.ask * 1.05, intent="init",
                        ...
                        # short if the stock is well above its normal values
                            ...
                            sell_order_id = order.algo_sell(self.symbol, algorithm=lime_midpoint_limit_sell,
                                                            price=md[self.symbol].L1.bid * .95, intent="init",
                                                            order_quantity=num_shares)
                            self.IsShort = True
                            self.entry_price = bar_close[0]
